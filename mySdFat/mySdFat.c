    ...
    if ((p_file.DIR_attr & ATTR_DIRECTORY) == 0)
    {
        if (p_file.DIR_ext[0] != ' ')
        {
            ...
            for (uint8_t index = 0; index < 3; index++)
                fileName[nameIndx++] =
                    (p_file.DIR_ext[index] == ' ') ? '\0' : p_file.DIR_ext[index] + 32;
        }
    }
}
...
static void dispFile(myFile p_file, char *fileName, uint8_t tab)
{
    ...
    /*
     ...
     debug_log_print(startCluster(p_file));
     */
}
...
static void fileSetDate(myFile *p_file, uint16_t year, uint8_t month,
                        uint8_t day)
{
    year -= 1980;
...
static void fileSetTime(myFile *p_file, uint8_t hours, uint8_t minutes,
                        uint8_t seconds)
{
...
bool fileWrite(myFile *p_file, const char *data)
{
         ...
         sector++)
    {
        ...
        if (!moreData)
        {
            ...
            if (SD_readSector(
                    startSecOfClus(p_file->fileEntInf.Cluster) + p_file->fileEntInf.sectorIndex, SD_buff) == SD_READ_SUCCESS)
            {
                myFile *p_temp = (myFile *)(SD_buff + p_file->fileEntInf.entryIndex * 32);
                memcpy(p_temp, p_file, 32);
                if (SD_writeSector(
                        startSecOfClus(p_file->fileEntInf.Cluster) + p_file->fileEntInf.sectorIndex, SD_buff) == SD_WRITE_SUCCESS)
                    return true;
            }
            return false;
        }
